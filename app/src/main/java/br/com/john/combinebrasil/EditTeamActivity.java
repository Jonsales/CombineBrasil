...
public class EditTeamActivity extends AppCompatActivity implements SingleUploadBroadcastReceiver.Delegate {
    Toolbar toolbar;
    ...
    private ConstraintLayout constraintProgress, constraintNotConnection;
    ...
    private Team team;
    private boolean canEditTeam = true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        act = EditTeamActivity.this;
        final View activityRootView = findViewById(R.id.activity_edit_team);
    ...
    private void getInfoTeam(){
        if(Services.isOnline(this)) {
            ...
            textProgress.setText(EditTeamActivity.this.getResources().getString(R.string.checking_team_information));
            String url = Constants.URL + Constants.API_TEAMS + "?" + Constants.TEAM_ID + "=" + idTeam;
            Connection task = new Connection(url, 0, Constants.GET_INFO_TEAM, false, EditTeamActivity.this);
            task.callByJsonStringRequest();
        }else
            constraintNotConnection.setVisibility(View.VISIBLE);
    }

    public static void retornaGetTeasmInformations(Activity act, String result, int status){
        ((EditTeamActivity)act).retornaGetTeasmInformations(result, status);
    }

    private void retornaGetTeasmInformations(String result, int status){
        ...
        if(status == 200 || status == 201) {
            try {
                ...
                if(teams!= null && teams.size()>0) {
                    ...
                    textAddress.setText(team.getAddres());
                    editTeam.setText(team.getName());
                    editEmail.setText(team.getEmail());
                    editNamePresident.setText(team.getPresidentName());
                    editSocialLink.setText(team.getSocialLink());
                    spinnerModality.setText(team.getModality());
                    ...
                    if(!team.getUser().isEmpty()) {
                        if(!team.getUser().equals(Services.getIdUser(this))) {
                            Services.messageAlert(this, getResources().getString(R.string.message), getResources().getString(R.string.mensagem_permissao_time), "");
                            canEditTeam = false;
                            return;
                        }
                    }
                ...
                Log.i("Error retorna Team", e.getMessage());
            }
        }
    }
    ...
    private void showImageTeam(String urlImage){
        Picasso.with(this)
            .load(urlImage)
            .into(imageTeam,  new ImageLoadedCallback(null) {
                @Override
            ...
            if (verifyFields()) {
                if (!spinnerModality.getText().toString().equals("")) {
                    if(canEditTeam == true)
                        ...
                        Services.messageAlert(EditTeamActivity.this, getResources().getString(R.string.message), getResources().getString(R.string.mensagem_permissao_time), "");
                } else
                    Services.messageAlert(EditTeamActivity.this, EditTeamActivity.this.getResources().getString(R.string.alert), EditTeamActivity.this.getResources().getString(R.string.select_mode), "");
            }
        }
    };

    private void editTeam() {
        if (Services.isOnline(EditTeamActivity.this)) {
            constraintProgress.setVisibility(View.VISIBLE);
            textProgress.setText(EditTeamActivity.this.getResources().getString(R.string.editing_team));

            String url = Constants.URL + Constants.API_TEAMS+"/"+idTeam;

            PutEditSelectiveAsyncTask put = new PutEditSelectiveAsyncTask();
            put.setActivity(EditTeamActivity.this);
            put.setObjPut(CreateJSON.createObjectTeam(createObjectTeam(), true));
            put.setMETHOD(Constants.METHOD_EDIT_TEAM);
            put.execute(url);
        }
    }

    public static void returnEditTeam(Activity act, String resp, String result) {
        ((EditTeamActivity) act).returnEditTeam(resp, result);
    }

    private void returnEditTeam(String resp, String result) {
        constraintProgress.setVisibility(View.GONE);
        if (result.toUpperCase().equals("OK")) {
            DeserializerJsonElements des = new DeserializerJsonElements(resp);
            team = des.getObjectTeam();
            if (isCropImage) {
                uploadImageTeam(team.getId(), team.getName());
            } else {
               updateTeam();
            }
        } else
    ...
    private void updateTeam(){
        ...
        db.updateTeam(team);
        Services.messageAlert(this, getResources().getString(R.string.message), getResources().getString(R.string.mensagem_equipe_atualizada), "exitTeam");
    }
    ...
    public static void returnMessage(Activity act, String methodCalled) {
        ((EditTeamActivity) act).returnMessage(methodCalled);
    }
    ...
    private void verifyError(String result) {
        try {
            if (result.contains("11000")) {
                Services.messageAlert(this, EditTeamActivity.this.getResources().getString(R.string.message), EditTeamActivity.this.getResources().getString(R.string.team_already_registered).replace("{TEAM}", editTeam.getText().toString()), "");
            } else
                Services.messageAlert(this, EditTeamActivity.this.getResources().getString(R.string.message), EditTeamActivity.this.getResources().getString(R.string.error_creating_team), "");
    ...
    private Team createObjectTeam() {
        ...
        team.setCode(this.team.getCode());
        team.setUser(this.team.getUser());
    ...
    private View.OnClickListener clickedDoneLocal = new View.OnClickListener() {
        ...
        if (!editCity.getText().toString().isEmpty()) {
            ...
            Toast.makeText(EditTeamActivity.this, EditTeamActivity.this.getResources().getString(R.string.must_fill_least_city), Toast.LENGTH_SHORT).show();
        }
    };
    ...
    private void searchAddress(String cep) {
        ...
        Connection task = new Connection(url, 0, Constants.CALLED_GET_CEP, false, EditTeamActivity.this);
        task.callByJsonStringRequest();
    }

    private void showLoadingAddress() {
        editCity.setText(EditTeamActivity.this.getResources().getString(R.string.location_city));
        editState.setText(EditTeamActivity.this.getResources().getString(R.string.finding));
        editStreet.setText(EditTeamActivity.this.getResources().getString(R.string.locatin_street));
        editNeighborhood.setText(EditTeamActivity.this.getResources().getString(R.string.locating_neighborhood));
    }

    public static void returnCEP(Activity act, String response, int statusCode) {
        ((EditTeamActivity) act).returnCEP(response, statusCode);
    }
...
// Add all other intents
    ...
    public void onProgress(long uploadedBytes, long totalBytes) {
        textProgress.setText(EditTeamActivity.this.getResources().getString(R.string.uploading_image));
    }
    ...
    public void onCompleted(int serverResponseCode, byte[] serverResponseBody) {
        ...
        if (file != null && file.exists())
            file.delete();
        Services.messageAlert(EditTeamActivity.this, getResources().getString(R.string.message), getResources().getString(R.string.mensagem_equipe_atualizada), "exitTeam");
    }
