...
public class AdapterRecyclerTestRecommended extends RecyclerView.Adapter<AdapterRecyclerTestRecommended.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        holder.txtInfoDefaultTest.setText(act.getString(R.string.test_matched_to) + " "+list.get(position).getValueType());
    ...
    private void verifyTestDefault(int position){
        if(list.get(position).isDefaultTest()){
            if(!list.get(position).isSelected()){
                ...
                int color = Color.argb(35, rnd.nextInt(256), rnd.nextInt(256), rnd.nextInt(256));
                for (int x = 0; x<=list.size()-1;x++ ) {
                    if(list.get(x).getId().equals(list.get(position).getSiblingTestType())){
                ...
                for (int x = 0; x<=list.size()-1;x++ ) {
                    if(list.get(x).getId().equals(list.get(position).getSiblingTestType())){
                        if(x<holderList.size())
                            holderList.get(x).listItem.setBackgroundColor(Color.rgb(255,255,255));
                    }
                }
            }
        }
